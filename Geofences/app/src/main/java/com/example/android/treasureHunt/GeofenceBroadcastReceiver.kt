    ...
    override fun onReceive(context: Context, intent: Intent)
    {
        ...
        if (intent.action == ACTION_GEOFENCE_EVENT)
        {
            ...
            if(geofencingEvent.hasError())
            {
                ...
                Log.i("Felo", errorMessage)
                return
            }

            if(geofencingEvent.geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER)
            {
                Log.i("Felo", context.getString(R.string.geofence_entered))
                val fenceId = when{
                    geofencingEvent .triggeringGeofences.isNotEmpty() -> geofencingEvent.triggeringGeofences[0].requestId
                    else -> {
                        Log.i("Felo", "No Geofence Trigger Found! Abort mission!")
                        return
                    }
                }
                ...
                if(-1 == foundIndex)
                {
                    Log.i("Felo", "Unknown Geofence Abort Mission!")
                    return
                }
